...
from sqlalchemy import BigInteger, Boolean, CheckConstraint, DateTime, Enum, ForeignKeyConstraint, Identity, Index, Integer, LargeBinary, PrimaryKeyConstraint, String, Text, UniqueConstraint, Uuid, text
...
class Users(Base):
    ...
    status: Mapped[AccountStatus] = mapped_column(Enum(AccountStatus, values_callable=lambda cls: [member.value for member in cls], name='account_status'), nullable=False, server_default=text("'active'::account_status"))
    ...
    privacy_settings: Mapped[Optional['PrivacySettings']] = relationship('PrivacySettings', back_populates='user', uselist=False)
...
class DeviceTokens(Base):
    ...
    token_id: Mapped[int] = mapped_column(Integer, Identity(), primary_key=True)
...
class PrivacySettings(Base):
    ...
    user: Mapped['Users'] = relationship('Users', back_populates='privacy_settings')
...
class Mentions(Base):
    ...
    mention_id: Mapped[int] = mapped_column(BigInteger, Identity(), primary_key=True)
